  ...
  return (
    <div className="min-h-screen bg-night-sky p-4 flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-7 gap-2 relative z-10">
        {/* Заголовки категорий */}
        {roundData.map((cat) => (
          <div key={cat.name} className="text-center bg-night/60 border border-gold/30 rounded-t-lg overflow-hidden flex flex-col justify-end">
            <div className="flex-1 overflow-hidden">
              <CategoryIcon name={cat.name} />
            </div>
            <div className="text-gold font-title text-2xl leading-tight px-1 py-0 flex items-center justify-center uppercase tracking-wider shrink-0">
              {cat.name}
            </div>
          </div>
        ))}
