      ...
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        <div className="space-y-2 mb-8">
          {sortedTeams.map((team, index) => {
            const placeEmoji = index === 0 && !team.eliminatedInRound ? '🥇' : index === 1 && !team.eliminatedInRound ? '🥈' : index === 2 && !team.eliminatedInRound ? '🥉' : '';
            return (
              <div
                key={team.id}
              ...
              >
                ...
                <div className="flex gap-2 text-xs">
                  {[1, 2, 3].map((r) => {
                    ...
                    const emoji = p === 1 ? '🥇' : p === 2 ? '🥈' : '🥉';
                    return (
                      <span key={r} className="px-1.5 py-0.5 rounded bg-night/40 text-pergament/70" title={`Раунд ${r}: ${p}-е место`}>
                        {roundLabels[r]} {emoji}
                      </span>
                    );
                  })}
